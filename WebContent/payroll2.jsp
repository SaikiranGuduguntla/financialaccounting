...
<%
    ...
    %>
		...
		<tbody>	
			...
			try
				{
					...
					String sql = "select * from consalary";
					rs=st.executeQuery(sql);
				while(rs.next())
					{
					%>
					<tr>
 					<td><%=rs.getString("conid") %></td>
 					<td><%=rs.getString("name") %></td>
      				<td><%=rs.getString("dept") %></td>
      				<td><%=rs.getString("basic") %></td>
      				<td><%=rs.getString("executive") %></td>
      				<td><%=rs.getString("conveyance") %></td>
			...
			<br><br><center>	
			<button><a href="payroll3.jsp">NEXT</a></button>
			</center>
			</div>
			</div>
			</div>
